...
from langfuse_utils import langfuse_chat_transformer
...
from serving_engine import LoRA, OpenAIServing
...
class OpenAIServingChat(OpenAIServing):
    ...
    @observe(
        ...
        capture_output=True,
        as_type="generation",
        transform_to_string=langfuse_chat_transformer,
    )
    async def create_chat_completion(
        ...
        # Some Langfuse updates to make request look better in UI
        langfuse_context.update_current_trace(
            name="create_chat_completion", version="vLLM", release=vllm.__version__
        )
        langfuse_context.update_current_observation(
            input=request, model=request.model, version="vLLM", release=vllm.__version__
        )
        ...
        try:
            ...
            guided_decode_logits_processor = await get_guided_decoding_logits_processor(
                guided_decoding_backend, request, await self.engine.get_tokenizer()
            )
        ...
        result_generator = self.engine.generate(
            prompt_text, sampling_params, request_id, prompt_ids, lora_request
        )
        # Streaming response
        if request.stream:
            # If we are using streaming Langfuse will handle it because of capture_output=True and we passed
            # transform_to_string=langfuse_chat_transformer
            ...
            try:
                full_generator = await self.chat_completion_full_generator(
                    request, raw_request, result_generator, request_id
                )
                langfuse_context.update_current_observation(
                    output=full_generator.model_dump()
                )
                ...
                langfuse_context.update_current_observation(
                    output=None,
                    status_message="Failed to process message. Something wrong with vLLM instance",
                )
                return self.create_error_response(str(e))
