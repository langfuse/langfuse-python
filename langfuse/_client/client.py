...
class Langfuse:
    ...
    Parameters:
        ...
        mask (Optional[MaskFunction]): Function to mask sensitive data synchronously when Langfuse SDK attributes are created. This applies only to data set through Langfuse SDK APIs such as `start_observation()`, `update()`, and `set_trace_io()`. If `mask_otel_spans` is not set and exported spans carry input or output attributes from other OpenTelemetry instrumentations, a one-time warning is logged.
